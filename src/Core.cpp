...
void Core::create_nodes(bool is_word_chain) {       /* 创建节点，生成图 */
    int cnt = 0;
...
void Core::dfs_all_chain(int id, vector<string> cur_chain) {
    vis[id] = true;
...
void Core::dfs_longest_chain(int id, int cur_v, vector<string> cur_chain) {
    cur_chain.push_back(nodes[id]->get_context());
...
Core::Core(const vector<string> &words, int words_size, bool enableLoop,
...
Core::Core(const vector<string> &words, int words_size) {
    this->words = words;
    this->words_size = words_size;
...
// 不要求和其他参数联合使用
int Core::genAllWordChain(vector<vector<string>> &result) {
    for (int i = 0; i < 26; i++) {
...
int Core::genMaxWordCountChain(vector<string> &result) {
    if (loop_exist) {
        for (int i = 0; i < nodes_size; i++) {
